...
def SRW_to_wind_data(filename):
    ...
    with open(filename) as file_in:
        ...
        # 載下的 .csv 與程式碼預期格式差異極大
        # sets appropriate field names for NASA POWER vs Wind Toolkit data
        if source == 'NASA/POWER':
            field_names = ('temperature', 'pres', 'speed', 'direction')
        else:
            unit = 100
            field_names = (f'air temperature at {unit}m (C)', f'air pressure at {unit}m (Pa)', f'wind speed at {unit}m (m/s)', f'wind direction at {unit}m (deg)')
        ...
        try:
            ...
            for row in reader:
                row = [i for i in row[5:] if i]
                data_dict['data'].append([float(i) for i in row])
            
            heights = [unit for _ in range(len(fields))] #remove empty strings
            data_dict['heights'] = [float(i) for i in heights]
            return data_dict
        except:
            unit = 140
            field_names = (f'air temperature at {unit}m (C)', f'air pressure at 100m (Pa)', f'wind speed at {unit}m (m/s)', f'wind direction at {unit}m (deg)')
            for field_name in fields:
                if field_name not in field_names:
                    raise ValueError(field_name + " required for wind data")
                data_dict['fields'].append(field_names.index(field_name) + 1)
...
class FetchResourceFiles():
    ...
    def _NSRDB_worker(self, job):
        ...
        if os.path.exists(file_path):
            ...
            lookup_query_url = f"nsrdb-data-query.json?api_key={self.nrel_api_key}&wkt=POINT({lon}%20{lat})"
            ...
            if lookup_response.ok:
                ...
                if ok:
                    ...
                    if data_response.ok:
                        ...
                        if self.verbose:
                            print('527 rows in ResourceTool.py: Success! File downloaded to {}.'.format(file_path))
                        return file_path
                    else:
                        try:
                        ...
                        print(f"534 rows in ResourceTool.py: Request failed for {data_url}\n{error_str}")
                        return
                else:
